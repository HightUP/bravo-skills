...
ACTIVE_SINCE_RX = re.compile(
    r"(?:Veiculação iniciada(?: em)?|Started running(?: on)?|Active since|Ativo desde)\s*[:\-]?\s*"
    r"(\d{1,2}\s+de\s+\w+\.?\s+de\s+\d{4}"     # "25 de set. de 2025"
    r"|\d{1,2}/\d{1,2}/\d{2,4}"                # "25/09/2025"
    r"|\w+\s+\d{1,2},\s*\d{4}"                 # "Sep 25, 2025"
    r"|\d{4}-\d{2}-\d{2})",                    # "2025-09-25"
    re.IGNORECASE,
)
...
def _extract_card(card) -> dict[str, Any]:
    ...
    meta_keywords = (
        ...
        "abrir menu",
        "open menu",
        "anúncios usam esse criativo",
        "usam esse criativo e esse texto",
        "ads use this creative",
        "esse anúncio tem várias versões",
        "this ad has multiple versions",
        "ver resumo do anúncio",
        "see ad summary",
        "página · ",
        "página criada",
        "page · ",
